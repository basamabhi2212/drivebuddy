...
function setupEventListeners() {
    ...
    const downloadDriverReportBtn = document.getElementById('downloadDriverReportBtn');
    if (downloadDriverReportBtn) {
        downloadDriverReportBtn.addEventListener('click', downloadDriverReport);
    }
        ...
        <div class="trip-card">
            ...
            <div class="trip-card-body">
                ...
                <div class="trip-detail">
                    ...
                    <span>${trip.assignedDriver ? drivers.find(d => d.id === trip.assignedDriver)?.name || 'Driver not found' : 'No driver assigned'}</span>
                </div>
                <div class="trip-detail">
                    <span>Status:</span>
                ...
                <div class="trip-actions">
                    ...
                    ${currentUser.role !== 'driver' && trip.status === 'active' ? `
                        <button class="btn btn-primary" onclick="editTrip('${trip.id}')">Edit</button>
                    ` : ''}
...
function loadDrivers() {
    ...
    driversContainer.innerHTML = drivers.map(driver => `
        <div class="driver-card">
            <div class="driver-card-header">
                Driver ID: ${driver.id}
                <span class="driver-status status-${driver.isEnabled ? 'enabled' : 'disabled'}">
            ...
            <div class="driver-card-body">
                ...
                <div class="driver-info">
                    <strong>License Expiry:</strong> ${driver.licenseExpiryDate || 'N/A'}
                </div>
                <div class="driver-info">
                    <strong>Bank:</strong> ${driver.bankName || 'N/A'}
                </div>
                <div class="driver-info">
                    <strong>IFSC:</strong> ${driver.ifscCode || 'N/A'}
                </div>
                <div class="driver-info">
                    <strong>Address:</strong> ${driver.address}
                </div>
                <div class="driver-info">
                    <strong>Added By:</strong> ${driver.addedBy}
                </div>
            </div>
            <div class="driver-actions">
                <button class="btn btn-secondary" onclick="editDriver('${driver.id}')">Edit</button>
                ${['admin', 'manager'].includes(currentUser.role) ? `
                    <button class="btn ${driver.isEnabled ? 'btn-warning' : 'btn-success'}" 
                            onclick="toggleDriverStatus('${driver.id}')">
                        ${driver.isEnabled ? 'Disable' : 'Enable'}
                    </button>
                ` : ''}
    ...
    // Update driver dropdown in trip form
...
function handleDriverAdd(event) {
    ...
    const bankDocument = document.getElementById('bankDocument').files[0];
    
    // Simulate file upload by creating a filename
    const bankDocumentName = bankDocument ? `bank-doc-${Date.now()}.${bankDocument.name.split('.').pop()}` : null;
    
    const driverData = {
        id: driverId || `DR${(drivers.length + 1).toString().padStart(3, '0')}`,
    ...
    alert(`Driver ${driverId ? 'updated' : 'added'} successfully! Driver ID: ${driverData.id}`);
}
...
function updateDriverDropdown() {
    ...
    assignedDriverSelect.innerHTML = '<option value="">No Driver Assigned</option>';
    
    // Add enabled drivers
    drivers.filter(driver => driver.isEnabled).forEach(driver => {
        const option = document.createElement('option');
        option.value = driver.id;
        option.textContent = `${driver.name} (${driver.mobile})`;
...
function toggleDriverStatus(driverId) {
    ...
    drivers[driverIndex].isEnabled = !drivers[driverIndex].isEnabled;
    ...
    alert(`Driver ${drivers[driverIndex].isEnabled ? 'enabled' : 'disabled'} successfully!`);
}

// Download Driver Report
function downloadDriverReport() {
    const activeDrivers = drivers.filter(driver => driver.isEnabled);
    
    if (activeDrivers.length === 0) {
        alert('No active drivers found');
        return;
    }
    
    // Create CSV content
    const csvHeader = 'Driver Name,Mobile Number,Bank Name,IFSC Code,Uploaded Document,License Number,License Expiry,Aadhar Number,Address,Added By,Added Date\n';
    const csvRows = activeDrivers.map(driver => {
        return [
            driver.name,
            driver.mobile,
            driver.bankName || 'N/A',
            driver.ifscCode || 'N/A',
            driver.bankDocument || 'N/A',
            driver.license,
            driver.licenseExpiryDate || 'N/A',
            driver.aadharNumber || 'N/A',
            driver.address.replace(/\n/g, ' '),
            driver.addedBy,
            new Date(driver.addedAt).toLocaleDateString()
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `Active_Drivers_Report_${new Date().toISOString().split('T')[0]}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
...
function editDriver(driverId) {
    ...
    document.getElementById('licenseIssueDate').value = driver.licenseIssueDate || '';
    document.getElementById('licenseExpiryDate').value = driver.licenseExpiryDate || '';
    document.getElementById('aadharNumber').value = driver.aadharNumber || '';
    document.getElementById('bankName').value = driver.bankName || '';
    document.getElementById('ifscCode').value = driver.ifscCode || '';
    document.getElementById('driverAddress').value = driver.address;
    
    // Update modal title and button
    document.querySelector('#driverModal .modal-header h3').textContent = 'Edit Driver';
    document.getElementById('submitDriverBtn').textContent = 'Update Driver';
    
    // Make bank document not required for edit
    document.getElementById('bankDocument').required = false;
...
function resetTripForm() {
    ...
    document.querySelector('#tripModal .modal-header h3').textContent = 'Book a Trip';
    document.getElementById('submitTripBtn').textContent = 'Book Trip';
    document.getElementById('dropLocationGroup').style.display = 'none';
    document.getElementById('dropLocation').required = false;
...
function editTrip(tripId) {
    ...
    document.getElementById('customerMobile').value = trip.customerMobile;
    ...
    document.getElementById('routeType').value = trip.routeType;
    document.getElementById('pickupLocation').value = trip.pickupLocation;
    document.getElementById('dropLocation').value = trip.dropLocation;
    document.getElementById('vehicleType').value = trip.vehicleType;
    document.getElementById('transmissionType').value = trip.transmissionType;
    document.getElementById('assignedDriver').value = trip.assignedDriver || '';
    
    // Handle route type visibility
    const dropLocationGroup = document.getElementById('dropLocationGroup');
    if (trip.routeType === 'one-way') {
        dropLocationGroup.style.display = 'block';
        document.getElementById('dropLocation').required = true;
    } else {
        dropLocationGroup.style.display = 'none';
        document.getElementById('dropLocation').required = false;
    }
    ...
    // Update modal title and button
    document.querySelector('#tripModal .modal-header h3').textContent = 'Edit Trip';
    document.getElementById('submitTripBtn').textContent = 'Update Trip';
